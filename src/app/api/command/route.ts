...
export async function GET() {
  return NextResponse.json({
    ...
    version: 2,
    endpoints: [
      { method: 'GET', path: '/api/command/state', body: null, does: 'full control-room state: beats, cast, voices, images, audio+manifest, engine runs, pulls, mined topics, formats+production_skins, health lamps', status: 'live' },
      ...
      { method: 'POST', path: '/api/command/showplan', body: '{formatId, runtime?, skinId?, seatOverrides?, guests?, input?}', does: 'STAGE 3 SHOWPLAN: compile the selected FORMAT + available cast into a role-assigned run of show (casting, branch-resolved beat sheet, render manifest, warnings). Compiler: lab/engine/showplan.mjs', status: 'live' },
