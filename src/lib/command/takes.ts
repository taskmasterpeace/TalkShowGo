...
export type Take = {
  ...
  client_key?: string; via?: string; created_at: string; used_in: string | null; used_at?: string; path: string
}
...
export function saveTake(beatId: string, person: { slug: string; name: string }, data: TakeInput, root = process.cwd()): Take {
  ...
  const obj = {
    ...
    voice: data.voice || null, ...(data.capped ? { capped: true } : {}), ...(data.depth ? { depth: String(data.depth) } : {}), ...(data.client_key ? { client_key: String(data.client_key).slice(0, 64) } : {}), ...(data.via ? { via: data.via } : {}),
