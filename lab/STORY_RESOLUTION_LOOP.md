...
## 2026-09-02 — the loop gets a Scout, a Log, human hands on the clusters, and a human Delegate
- **THE SCOUT** (`src/lib/command/scout.ts`, `/api/command/scout`, SOURCES page): topic-first source discovery. Given a beat + a topic ("Lil Durk case"), aggregates who is covering it on YouTube (Innertube, freshness + relevance passes) and X (advanced_search within the window), ranks by real signal, marks `IN BEAT` vs `+ ADD` (one click writes the beat), resolves a bare name to a channel, and guards squatter channels (a 93-sub "Akademiks" is `SUSPECT`, alternates shown). Proven: Ceddy Nash / King Akademiks IN BEAT, CUFBOYS / The Akademy / Tales From Jails / djvlad suggested; SM16 weekend → Matter of Factz, Battle Rap Science, Convos in the Cave, QuickHitsTV.
- **ACTIVITY LOG** (`src/lib/command/log.ts`, `/api/command/log`, `/command/log`): every pipeline action is one JSONL line (`lab/logs/activity.jsonl`, gitignored) with facet filters (kind, beat, window, errors, text) and a LIVE toggle. make_show appends the same format.
- **HUMAN-IN-THE-LOOP CLUSTERING** (`cluster-overrides.ts`, `/api/command/cluster/edit`, `Clusters.tsx`): pin / rename / merge / split / dismiss / undo as an append-only op-log replayed over the AI's clusters on every read (stale-detected if the pull is re-clustered); the producer ranks the human view and a PINNED story goes first regardless of score.
- **THE DELEGATE, HUMAN PATH** (`/api/command/briefing/interview`, STRINGER delegate panel): the show writes 4-6 neutral, tailored questions from the briefing; the person answers in their own words; saved VERBATIM as a `human:true` delegate delivery that a host re-brief can never delete. (Seating human delegates on the floor as verbatim turns is the next step — see participants below.)
- **Durable shows**: unique job dirs, heartbeat-based liveness, cancel (process tree), resume-from-stage, atomic status, SHOWS on TAPE, persisted one-click; **renderer parser fix** (bracket tags were silently dropping 2/3 of lines); **PULL** `{hours}` override + yt-dlp fallback + unique filenames.

## 2026-09-02 (cont.) — YouTube solved, delegates SEATED on the floor, DATAFLOW, Scout auto-add
- **YOUTUBE RELIABILITY** (`src/lib/command/yt.ts` + PULL route): the ids were never stale (Innertube resolves all six), YouTube's RSS just throttles bursts. The pull now walks three rungs per channel: **RSS** (exact timestamps) → **Innertube** Videos-tab (keyless, in-process, LockupView nodes with "N ago" stamps parsed to `age_hours`, filtered strictly `< hours`) → **yt-dlp** flat listing. A channel Innertube cannot open at all is a dead/renamed id: the pull **re-resolves it by name and repairs the beat** (re-reads the beat first and patches only that channel_id, so a concurrent scout add is never clobbered). `POST {skip_rss:true}` forces the Innertube rung. DESK meter and the log report which rung answered. Proven on the 48h battle-rap beat: 6/6 channels return, mix of `rss`/`innertube`.
- **DELEGATES ON THE FLOOR** (`compile_beat.mjs` / `run_floor.mjs` / `render_breeze.mjs` / `render_kokoro.mjs`): the beat card now emits `participants`, `delegates.{ai,human}`, and `human_slots`. An **AI delegate** takes the floor as a synthetic host on its own DNA engine (its briefed stance + allowed evidence, max 2 seats). A **HUMAN delegate** is seated **verbatim** — their interview answers become scripted turns dropped at word marks across the floor, a host is directed to answer them by name, and the MIX pass is rejected if it alters a verbatim line. Voices: human → **Breeze clone on the person's OWN recording** (`voice.sample_wav` + `ref_text`); AI or a sample-less human → a designed guest voice cached once (`lab/cast/voices/delegate-<slug>.wav`); Kokoro draft path gives each delegate an unused voice. Shows WITHOUT delegates are unchanged (no participants ⇒ house hosts only). **Proven end to end**: recorded take → Gemini verbatim transcript → multiple-choice follow-ups → saved human delegate → seated 3× on the floor → Breeze cloned that same recording for every turn (1:20 mp3).
- **DELEGATE VOICE PATH** (`src/lib/command/stt.ts`, interview route, STRINGER panel): RECORD a spoken answer → ffmpeg to 24kHz mono wav → OpenRouter Gemini `input_audio` verbatim transcript (~1.4s) into an editable box → one flash-lite call writes 2-3 follow-ups grounded in what was said, each as tappable choices + a free-text box → THAT IS MY TAKE saves every answer verbatim with its `source`, and the longest take + its transcript ride the delivery as `voice` for the floor to clone. Path-guarded (`ownedWav`: a wav is honored only inside this delegate's take folder).
- **SCOUT AUTO-ADD** (`/api/command/scout`, SOURCES page): with a beat selected, any YouTube channel with **3+ videos on the topic inside the window**, an author-stamped real UC id, not already in the beat and not an excluded outlet, is written into the beat immediately (status `RESOLVED <date> (scout auto: N vids on "<topic>")`), through the same helper the manual ADD uses. Persisted `AUTO-ADD 3+ VIDS` toggle; X never auto-adds (a handle needs the verify flow's userId).
- **DATAFLOW** (`src/lib/command/dataflow.ts`, `/api/command/dataflow`, `/command/dataflow`): "how the data travels" — PULL → CLUSTER → LEADS → RANK → DOSSIERS → BRIEFINGS → STANCES → SHOWS as a scrollable strip, each hop with real counts, top-5 samples, file, timestamp, ms; click any sample to light its full lineage across the strip and open an inspector; TRACE ONE STORY narrows to a single event's journey; a CLOCK table of log timings.
- **Beats renamed** (channel-forward): battle-rap → **"Algorithm Institute of Battle Rap"** (48h default), hood → **"Hood History Club"**, Orangeburg stays (testing). Beat ids unchanged.
- **Robustness**: lead miner recovers a bare/`truncated` JSON array (8k token cap) so a big feed no longer yields 0 leads; producer-rank retries once on an empty completion before it reaches the desk.
