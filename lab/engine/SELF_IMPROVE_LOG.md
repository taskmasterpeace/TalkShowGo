...
| 8 | (next) **DIRECTOR side-split — now viable because evidence is two-sided.** In `compile_beat` director: on a yes/no question, force the two debaters onto CLEARLY OPPOSING verdicts (one argues YES/SET, one argues NO/PROBLEM), each handed the receipts that back THEIR side (split the ATTRIBUTED_CLAIMs by which verdict they support). Also forbid empty moderator turns (a moderator turn must ask/press a NAMED claim, never "Mm"/"Right" — likely a small `run_floor` guard: a moderator/anchor-lane turn under ~5 words with no "?" is rejected). Verify on the QB floor (heat should rise from 2; overall past the 4-5 plateau). | — | — | P3 efficiency |
