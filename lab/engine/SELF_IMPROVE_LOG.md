...
| 3 | (next) BEGIN P2 — SEGMENTS (the headline). Collision is a working baseline; segments is what Robert wants. Design + first cut of a multi-story episode: pick 2-3 mined stories, mini-floor each, stitch with a host-written transition, one intro/outro, one leveled mp3. Start with make_show/compile plumbing to accept a segment list. | — | — | P2 cont. |
