...
async function main() {
  ...
  for (const n of debaters) {
    ...
    preds.push([n, await line(MODEL, `You are ${h.name}, a host on "${showName}". WHO YOU ARE: ${(p.essence || '').slice(0, 300)} YOUR VOICE: ${(p.speech && p.speech.tone) || ''}. This is the CLOSING ROUND - the show is ENDING: NO greeting, NO "welcome back", NO introducing yourself or the show, NO recapping tonight; the moderator just tossed to you, so your FIRST sentence IS the prediction. Write ONE bold, SPECIFIC, ownable PREDICTION about tonight's stories - a real call the comments can hold you to (a number or an outcome; a person's or TEAM's name ONLY if the show actually discussed them - NEVER invent a coach, player, or upcoming opponent the show did not establish tonight). It MUST differ from any prediction already on the record (listed below). NEVER name a calendar year - say "this season" or "next week". Stay consistent with what the show established tonight (below), especially who the next opponent is. 1-2 spoken sentences, committed, no hedging, no em-dashes.`, `Tonight's stories:\n${topics.join('\n')}\nEstablished on the show tonight (your call must not contradict these):\n${segs.map(s => '- ' + lastLineOf(s.md)).join('\n')}${preds.length ? '\nAlready on the record (yours must DIFFER): ' + preds.map(x => x[1]).join(' | ') : ''}`)])
  }
